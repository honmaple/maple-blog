...
class BaseForm(Form):
    name = StringField('用户名:', [DataRequired(), Length(min=4, max=20)])
    passwd = PasswordField('密码:', [DataRequired(), Length(min=4, max=20)])
...
class ForgetPasswdForm(Form):
    confirm_email = StringField('注册邮箱:', [DataRequired(), Email()])
    code = StringField('验证码:', [DataRequired(), Length(min=4, max=4)])
