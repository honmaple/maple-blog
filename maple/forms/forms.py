...
class Length(object):
    ...
    def __call__(self, form, field):
        ...
        if l < self.min or self.max != -1 and l > self.max:
            ...
            if message is None:
                if self.max == -1:
                    message = field.ngettext('输入长度至少 %(min)d 个字符.',
                                             '输入长度至少 %(min)d 个字符.', self.min)
                elif self.min == -1:
                    message = field.ngettext('输入长度不能超过 %(min)d 个字符.',
                                             '输入长度不能超过 %(min)d 个字符.', self.max)
                else:
                    message = field.gettext('输入长度应在 %(min)d 到 %(max)d 个字符之间.')
...
class EqualTo(object):
    ...
    def __call__(self, form, field):
        ...
        if field.data != other.data:
            d = {'other_label': hasattr(other, 'label') and other.label.text or
                 self.fieldname,
                 'other_name': self.fieldname}
            message = self.message
            if message is None:
                message = field.gettext('输入必须与 %(other_name)s 相同.')
